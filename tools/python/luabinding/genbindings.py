...
def _check_cocos2dx_root_env():
    ''' Checking the environment COCOS_X_ROOT, which will be used for building
    '''

    try:
        COCOS_X_ROOT = os.environ['COCOS_X_ROOT']
    except Exception:
        print "COCOS_X_ROOT not defined,Please define COCOS_X_ROOT in your environment."
        sys.exit(1)
...
def main():
    ...
    if os.path.isdir(x86_llvm_path):
        ...
        print 'path: %s or path: %s are not valid! ' % (x64_llvm_path, x64_llvm_path)
        sys.exit(1)


    project_root = _check_cocos2dx_root_env()
    cocos_root = os.path.abspath(os.path.join(project_root, ''))
    cxx_generator_root = os.path.abspath(os.path.join(project_root, 'tools/bindings-generator'))
    classdir = os.path.abspath(os.path.join(project_root, 'tests/lua-empty-test/project/Classes'))
    ...
    config.set('DEFAULT', 'classdir', classdir)
    config.set('DEFAULT', 'cocos2dxdir', cocos_root)
    ...
    if '3.3' in llvm_path:
    	...
    	config.set('DEFAULT', 'clang_version', '5.0')
    	config.set('DEFAULT', 'clang_lib_version', 'lib')
    ...
    try:
        ...
        tolua_root = os.path.abspath(os.path.join(os.path.dirname(__file__), 'ini'))
        output_dir = '%s/gamecore/tolua' % classdir

        cmd_args = {'lua_gamecore.ini' : ('lua_gamecore', 'lua_gamecore_auto')
